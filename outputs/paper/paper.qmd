...
library(scales)
### Input data ###
single_use_data <- read_xlsx(here::here("outputs/data/single_use_data_2018.xlsx"))
single_use_data <- read_xlsx(here::here("outputs/data/single_use_data_2019.xlsx"))
```
...
frequency_table_hot <- single_use_data %>%
  group_by(`Hot To-Go Cups`) %>%
  summarise(Freq = n()) %>%
  as.data.frame()

# Plot the bar chart
hot_to_go <- ggplot(data = frequency_table_hot, aes(x = `Hot To-Go Cups`, y = Freq)) +
